function [Mass,Force,Fine,Fint,Fext] = ...
	SuperTrussElement_CubicS
pline_MassForce(...
	qe,dqe,g,Truss_Parameter)
%%
...
% J = diag([Iy+Iz,Iz,Iy]);
%
EquivalentParameter = Truss_Parameter.EquivalentParameter;
EquivalentDynamicParameter = ...
	EquivalentParameter.Dynamic;
EquivalentStaticParameter = ...
	EquivalentParameter.Static;
lambda_x_polyfunc = ...
	EquivalentDynamicParameter.lambda_x_polyfunc;
% lambda_y_polyfunc = EquivalentParameter.lambda_y_polyfunc;
% lambda_z_polyfunc = EquivalentParameter.lambda_z_polyfunc;

rhoJ_x_polyfunc = ...
	EquivalentDynamicParameter.rhoJ_x_polyfunc;
rhoJ_y_polyfunc = ...
	EquivalentDynamicParameter.rhoJ_y_polyfunc;
rhoJ_z_polyfunc = ...
	EquivalentDynamicParameter.rhoJ_z_polyfunc;

EA = EquivalentStaticParameter.EA_Stretch;
GIyIz = EquivalentStaticParameter.GIyIz;
EIy = EquivalentStaticParameter.EIy;
EIz = EquivalentStaticParameter.EIz;
Stiffness = diag([EA,GIyIz,EIy,EIz]);
%%
q1 = qe(1:numel(qe)/2);
...
D = ...
	...
	zeros(3),-skew(dr1dx)*R1,zeros(3),zeros(3);
	zeros(3),zeros(3),eye(3),zeros(3);
	zeros(3),zeros(3),zeros(3),-skew(dr2dx)*R2];
dDdt = ...
	[zeros(3),zeros(3),zeros(3),zeros(3);
	zeros(3),-norm_dr1dx*R1*skew(omega1)*skew(gx),zeros(3),zeros(3);
	zeros(3),zeros(3),zeros(3),zeros(3);
	zeros(3),zeros(3),zeros(3),-norm_dr2dx*R2*skew(omega2)*skew(gx)];
%%
gaussn = 5;
...
for i = 1:gaussn
	...
	Rx = R1 * R_1_x;
	%%
	lambda_x = polyval(lambda_x_polyfunc,x);
% 	lambda_y = polyval(lambda_y_polyfunc,x);
% 	lambda_z = polyval(lambda_z_polyfunc,x);
	
	lambda = diag([lambda_x,lambda_x,lambda_x]);
	lambda = Rx * lambda * Rx';
	%
	rhoJ_x = polyval(rhoJ_x_polyfunc,x);
	rhoJ_y = polyval(rhoJ_y_polyfunc,x);
	rhoJ_z = polyval(rhoJ_z_polyfunc,x);
	rhoJ = diag([rhoJ_x,rhoJ_y,rhoJ_z]);
	%%
	epsilon = norm(drdx) - 1;
