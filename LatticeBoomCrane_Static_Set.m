...
if do_ResultMerge
	...
	for MainBoomAngleNr = 1:10
		for deltaLNr = 1:32
			pos_static_all{MainBoomAngleNr,deltaLNr} = ...
				pos_static{MainBoomAngleNr,deltaLNr};
		end
	end
	load('Result\Lattice Boom Crane 100t Static Position\Temp_11_20.mat','pos_static');
	for MainBoomAngleNr = 11:20
		for deltaLNr = 1:32
			pos_static_all{MainBoomAngleNr,deltaLNr} = ...
				pos_static{MainBoomAngleNr,deltaLNr};
		end
	end
	load('Result\Lattice Boom Crane 100t Static Position\Temp_21_30.mat','pos_static');
	for MainBoomAngleNr = 21:30
		for deltaLNr = 1:32
			pos_static_all{MainBoomAngleNr,deltaLNr} = ...
				pos_static{MainBoomAngleNr,deltaLNr};
		end
	end
	load('Result\Lattice Boom Crane 100t Static Position\Temp_31_41.mat','pos_static');
	for MainBoomAngleNr = 31:41
		for deltaLNr = 1:32
			pos_static_all{MainBoomAngleNr,deltaLNr} = ...
				pos_static{MainBoomAngleNr,deltaLNr};
		end
	...
	save('Result\Lattice Boom Crane 100t Static Position\pos_static.mat','pos_static');
else
	load('Result\Lattice Boom Crane 100t Static Position\pos_static.mat','pos_static');
end
%% Test
...
if do_VideoValidation
	...
	VideoObj = VideoWriter('Video\LatticeBoomCrane_StaticPositionValidation_100t.avi');
...
load('Result\Lattice Boom Crane 100t Static Position\LatticeBoomCrane_EndPoint_Trajectory2.mat', ...
	'OptimalTrajectory');
delta_t = 0.02;
...
tSet_FitData = [...
	tSet_FitData(1)+(linspace(-5,-1,CableLength_FitData_ExpandQuantity))'; ...
	tSet_FitData;
	tSet_FitData(end)+(linspace(1,5,CableLength_FitData_ExpandQuantity))'];
...
save('Result\Lattice Boom Crane 100t Static Position\ControlVariable_Func.mat', ...
...
%%
clear;close all;
load('Result\Lattice Boom Crane\Control\100t\Verify Revised\Result.mat', ...
	't_set','x_set');
