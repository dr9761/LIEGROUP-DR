% DoubleJib_Pendulum_Symbolic_Test
%%
clc;clear;close all;
ExcelFileName = ...
	'Double Jib Pendulum';
...
%%
q = casadi.SX.sym('q',4,1);
dq = casadi.SX.sym('dq',4,1);
u = casadi.SX.sym('u',2,1);
%%
t = 0;
x = [q;dq];

[dx,Mass,Force] = DoubleJib_Pendulum_Dynamic_func_Symbolic(...
	x,u,ModelParameter);
%%
ddxdx = jacobian(dx,x);
ddxdu = jacobian(dx,u);

MassJacobianBase = casadi.SX.sym('b',4,1);
...
phi_y = 0;
phi_z = 0;
qn = [phi_s;phi_1;phi_y;phi_z];
%
dphi_sdt = 0;
dphi_1dt = 0;
dphi_ydt = 0;
dphi_zdt = 0;
dqn = [dphi_sdt;dphi_1dt;dphi_ydt;dphi_zdt];
%
un = [0;0];
%
xn = [qn;dqn];
MassJacobianBase_n = [1;1;1;1];
%%
