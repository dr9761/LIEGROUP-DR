function cos_Substitute2(t,x_range,t_range)
if nargin == 0
    t = [];
    x_range = [10,20];
    t_range = [5,15];
...
for t_Nr = 1:numel(t_set)
    ...
    elseif t > t_end
        ...
        x = x_start + ...
            1/2*(x_end-x_start)*(1-cos(2*pi*(t-t_start)/(t_end-t_start)));
        dxdt = -pi*(x_start-x_end)/(t_start-t_end)*sin(2*pi*(t-t_start)/(t_end-t_start));
        ddxdtdt = -2*pi^2*(x_start-x_end)/((t_start-t_end)^2)*cos(2*pi*(t-t_start)/(t_end-t_start));
    end
